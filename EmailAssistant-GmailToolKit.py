# This code is a development from the RAGEmailAssistant which uses the graphRAG. Here we will add the langchain gmail
# tool kit inorder to write the drafts
from dotenv import load_dotenv
from langchain_community.tools import GmailCreateDraft
from langchain_community.tools import GmailSearch
import time

from GraphRAG import chain as ragchain
...
prompt = PromptTemplate(
    ...
    You are the Email Categorizer Agent for the Community Sector Council of Newfoundland and Labrador (CSCNL),You are a master at \
    understanding what a customer wants when they write an email and are able to categorize \
    it in a useful way. Remember people maybe asking about the services provided and notable work done by CSCNL. \
    ...
    Conduct a comprehensive analysis of the email provided and categorize into one of the following categories:
        ...
        service_enquiry - used when someone is asking for information about the services provided by CSCNL
        notable_work - used when someone is asking for information about the achievements or work done by CSCNL
        general - used when someone is asking for information about CSCNL that doesn't fall into any other category
        customer_feedback - used when someone is giving feedback about a product \
        off_topic when it doesnt relate to CSCNL\
...
search_rag_prompt = PromptTemplate(
    ...
    info for helping to write the final email. Remember when people ask about a generic Council they are probably
     referring to the Community Sector Council of Newfoundland and Labrador. Write the questions to our knowledge system not to the customer.
...
draft_writer_prompt = PromptTemplate(
    template="""<|begin_of_text|><|start_header_id|>system<|end_header_id|>
    You are the Email Writer Agent for the Community Sector Council of Newfoundland and Labrador (CSCNL), take the INITIAL_EMAIL below \
    ...
    about services the company has and the work it has done.
            ...
            Always sign off the emails in appropriate manner and from Mark, the Customer Care Associate.
...
def research_info_search(state):
    ...
    for question in questions:
        print(question)
        temp_docs = ragchain.invoke(question)
        print(temp_docs)
...
def write_draft_email(state):
    """Writes the draft email in the user Gmail"""
    print("---WRITING DRAFTED EMAIL TO GMAIL---")
    message = state['final_email']
    email = state['sender_address']
...
workflow.add_node("write_draft_email", write_draft_email)
"""### Add Edges"""
...
workflow.add_edge("state_printer", "write_draft_email")
workflow.add_edge('write_draft_email', END)

# Compile
app = workflow.compile()


# Creating the 2nd workflow
class Second_State(TypedDict):
    is_new_email: bool
    checked_email_ids: List[str]


# defining the nodes for this graph
# Node 1: The check new email node
def check_new_email(state):
    gmail = GmailToolkit()
    search = GmailSearch(api_resource=gmail.api_resource)
    emails = search('after:newer_than:1d')
    is_new_email = state['is_new_email']
    checked_email_ids = state['checked_email_ids']
    for email in emails:
        if email['id'] not in checked_email_ids:
            is_new_email = True
    return {'is_new_email': is_new_email, 'checked_email_ids': checked_email_ids}
...
def write_emails(state):
    ...
    for email in emails:
        if email['id'] not in checked_email_ids:
            ...
            inputs = {'initial_email': message, 'sender_address': sender, 'email_subject': subject}
            app.invoke(inputs)
            checked_email_ids.append(email['id'])
    return {'checked_email_ids': checked_email_ids}


# Conditional edges
def route_to_write(state):
    if state['is_new_emails']:
        return 'write'
    else:
        return 'wait'


# creating the outer langGraph


outer_work_flow = StateGraph(Second_State)
# adding the nodes
outer_work_flow.add_node('check_new_emails', check_new_email)
outer_work_flow.add_node('write_emails', write_emails)
outer_work_flow.add_node('wait', wait)
# adding the unconditional edges
outer_work_flow.add_edge('write_emails', 'wait')
outer_work_flow.add_edge('wait', 'check_new_emails')
# adding the conditional edges
outer_work_flow.add_conditional_edges('check_new_emails', route_to_write, {
    'write': 'write_emails', 'wait': 'wait'
})
outer_work_flow.set_entry_point('check_new_emails')
outerApp = outer_work_flow.compile()
inputs = {"is_new_email":False , "checked_email_ids":[]}
outerApp.invoke(inputs)
